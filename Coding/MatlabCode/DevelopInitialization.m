...
for ID=1:InitialPar.NumberofNeurons
    ...
    for IDC=1:RealGrainedN
        for IDC2=1:RealGrainedN
                ...
                % In the above cell, the 1st column saves the coordinates of synaptic components;
                % The 2nd column saves the calcium on synaptic components;
                % The 3rd column saves the Tubulin concentration on synaptic components;
                % The 4th column saves UMAP-2 concentration on synaptic components;
                % The 5th column saves BMAP-2 concentration on synaptic components;
                % The 6th column saves PMAP-2 concentration on synaptic components; 
                % The 7th column saves the elongation length on synaptic components;
                % The 8th column saves the branching probability;
                % The 9th column saves the radius of synapse;
                % The 10th column saves the segment type (1 stands for root segment, 
                % 2 stands for intermediate segment, 3 stands for terminal segment);
                % The 11th column saves the previous segment; 
                % The 12th column saves the next segment (or segments if there is branching); 
                % The 13th column saves the centrifugal order;
                % The 14thcolumn saves temporary information
                for IDC3=2:9
                ...
                DevelopmentInfoCell{ID,4}{IDC,IDC2}=zeros(InitialPar.DurationLength,4); 
                % the 1th column saves the average elongation rate, 
                % the 2nd column saves the variance of elongation rate,
                % the 3rd column saves the number of terminal segments; 
                % the 4th column saves the normalization base of centrifugal order;
